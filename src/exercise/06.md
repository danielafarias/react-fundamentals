...
- O atributo onSubmit irá chamar o evento de envio, na qual há um alvo (target), esse target é uma referência para o nó (node) formulário (<form>) do DOM para conseguir os valores do formulário
- Ref referencia o valor atual (current) do elemento sem a necessidade de renderização. No React é utilizado o hook useRef
- Componentes de Função não podem receber refs, pois o ref não consegue acessar diretamente ao DOM, havendo a necessidade de usar o forwardRef no componente
- Você não consegue ler ou escrever o current ref durante a renderização
- Ref não é útil quando há a necessidade de display do valor, tendo que utilizar o useState
- Utilize ref quando é necessário guardar uma valor na qual não impacte na renderização
- O hook useState permite a criação e uso da variável de estado (state variable)
- State (estado) é resumidademente uma memória de um componente específico
- useState é um array com dois valores: O estado atual (inital ou current) e a função set que atualiza o valor do estado e chama a renderização novamente
- Evitar utilizar useState dentro de laços diretamente
- Dentro da função set é possível pegar o valor anterior (previous state)
- Se caso o novo valor for igual ao antigo o componente não sofre uma nova renderização (em casos excepcionais, sim)
- flushSync: Atualiza o DOM imediatamente de maneiro síncrona
- Updater function: utilizando a função set com o previous state
- Armazenar uma função em um estado é necessário utilizar arrow function
