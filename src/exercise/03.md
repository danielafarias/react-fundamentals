...
- JSX não é exatamente o JavaScript, é necessário converte-lo utilizando compiladores, como o Babel.
- Usar a prática de sempre compilar para ver o resultado no [playground do Babel](https://babeljs.io/repl#?browsers=defaults%2C%20not%20ie%2011%2C%20not%20ie_mob%2011&build=&builtIns=App&corejs=3.21&spec=false&loose=false&code_lz=MYewdgzgLgBArgSxgXhgHgCYIG4D40QAOAhmLgBICmANtSGgPRGm7rNkDqIATtRo-3wMseAFBA&debug=false&forceAllTransforms=false&modules=false&shippedProposals=false&circleciRepo=&evaluate=false&fileSize=false&timeTravel=false&sourceType=module&lineWrap=true&presets=react&prettier=true&targets=&version=7.22.20&externalPlugins=&assumptions=%7B%7D)
- JSX é uma extensão de sintaxe para JavaScript na qual permite escrever uma linguagem de marcação similar ao HTML dentro do arquivo JavaScript
- Frequentemente o React é utilizado junto ao JSX, mas podem ser usados separadamente
- Regras mais restritas existentes no JSX que não existe no HTML:
    - Retorne sempre um único elemento no root, para retornar diversos elementos em um componente é necessário envolver eles em um elemento ou fragment
