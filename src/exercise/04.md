...
- Componentes são basicamente funções que retorna alguma coisa que possa renderizar (React Elements e etc)
- É possivel utilizar o React.createElement() chamando uma função que retorna um JSX e colocando uma propriedade nesse JSX
- Utilizar primeira letra maiuscula ao usar a função JSX para retorno de um componente como tag
- prop-types é uma biblioteca que declara tipagem aceitas as propriedades do componente. Esses tipos serão verificados durante a renderização em ambiente de desenvolvimento (local)
- Fragment não é util para estilização
- É possível adicionar key no Fragment caso declarar de fato que é um fragment, exemplo: <Fragment key={i} />
- Ao usar elementos em volto de um fragment no DOM, o fragment não significará nada, como se os elementos tivessem sido postos diretos no corpo
