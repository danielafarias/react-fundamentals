...
- ******************************O que é o DOM?****************************** É uma interface de programação para os documentos HTML e XML, na qual representa o documento orientado a objetos (com nós e objetos), para que as linguagens de programação (script) podem se conectar à página
- A maioria dos navegadores modernos utilizam W3C DOM e WHATWG DOM
- O DOM moderno é construído usando várias API. O DOM principal define os objetos que descrevem um documento e os objetos dentro dele. Isso é expandido conforme necessário por outras APIs que adicionam novos recursos e capacidades ao DOM. Por exemplo, a HTML DOM API adiciona suporte para representar documentos HTML no DOM principal.
- API (Página HTML ou XML) = DOM + JavaScript
