...
- Utilização da key para manipular renderização de arrays por ordem
- Sem uma key = perde a posição do elemento na renderização
- Com o index de uma array como key = sempre será alterado o a partir da posição do array e nem sempre o elemento correto
- Com o id como key = sempre indetificará o elemento independentemente da posição que ele vá
